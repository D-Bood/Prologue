...
class Links_Plugin implements Typecho_Plugin_Interface
{
	...
	public static function output_str($pattern=NULL, $links_num=0, $sort=NULL)
	{
		...
		if (!isset($pattern) || $pattern == "" || $pattern == NULL || $pattern == "SHOW_TEXT") {
			$pattern = "<div class=\"linkitem\"><a href=\"{url}\" title=\"{title}\" target=\"_blank\">{name}</a></div>\n";
		} else if ($pattern == "SHOW_IMG") {
			$pattern = "<div class=\"linkitem\"><a href=\"{url}\" title=\"{title}\" target=\"_blank\"><img class=\"linkimg\" src=\"{image}\" alt=\"{name}\" /></a></div>\n";
		} else if ($pattern == "SHOW_MIX") {
			$pattern = "<div class=\"linkitem\"><a href=\"{url}\" title=\"{title}\" target=\"_blank\"><img class=\"linkimg\" src=\"{image}\" alt=\"{name}\" /><p>{name}</p></a></div>\n";
                } else if ($pattern == "SHOW_HTTPS") {
                        $pattern = "<div class=\"linkitem\"><a href=\"{url}\" title=\"{title}\" target=\"_blank\"><img class=\"linkimg\" src=\"{image}\" alt=\"{name}\" /><p><i class=\"https fa-lock\"></i> {name}</p></a></div>\n";
		}
		$db = Typecho_Db::get();
		$prefix = $db->getPrefix();
		$options = Typecho_Widget::widget('Widget_Options');
